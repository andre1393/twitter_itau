...
public class Reports {
	...
	/**
	 * Dada uma lista dos resulados das pesquisas das #tags, constrói uma string
	...
	/**
	 * Dado um array de 24 posições (um para cada hora do dia), constroi uma string em 
	 * formato json para apresentar a quantidade de tweets totais por hora do dia
	 * @param arrayHours - array de inteiro de 24 posições que contem a quantidade total
	...
	public String getTotalHours(int[] arrayHours)
	{
		...
		for(int i = 0; i < arrayHours.length; i++)
		{
			...
			// transforma os parametros/valores em string (json)
			totalHours += Support.getJson(totalTagParams, values);
